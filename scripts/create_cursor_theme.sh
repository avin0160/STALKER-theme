...
def create_drag_drop(size=32):
    """Solid thick nut/bolt cursor for drag and drop"""
    img = Image.new('RGBA', (size, size), (0, 0, 0, 0))
    draw = ImageDraw.Draw(img)
    
    center_x, center_y = size // 2, size // 2
    
    # Thick hexagonal nut (solid filled)
    hex_size = 9
    
    # Draw hexagon as solid nut
    angles = [i * 60 for i in range(6)]
    points = []
    for angle in angles:
        rad = math.radians(angle)
    ...
    for angle in angles:
        rad = math.radians(angle)
        x = center_x + (hex_size + 1) * math.cos(rad) + 1
        y = center_y + (hex_size + 1) * math.sin(rad) + 1
        shadow_points.append((x, y))
    draw.polygon(shadow_points, fill=(0, 0, 0, 100))
    
    # Solid filled hexagon (dark STALKER green)
    draw.polygon(points, fill=STALKER_DARK, outline=None)
    
    # Thick outline
    draw.polygon(points, outline=STALKER_PRIMARY, width=3)
    
    # Center hole (like a bolt/nut)
    hole_size = 4
    draw.ellipse([
        center_x - hole_size, center_y - hole_size,
        center_x + hole_size, center_y + hole_size
    ], fill=(0, 0, 0, 180), outline=STALKER_ACCENT[:3] + (200,), width=1)
    
    # Bright green center dot (smaller, inside hole)
    dot_size = 2
    draw.ellipse([
    ...
    # Add grip lines on the hexagon (like a real nut)
    grip_color = STALKER_ACCENT[:3] + (150,)
    for i in range(6):
        angle = i * 60 + 30  # Between vertices
        rad = math.radians(angle)
        start_dist = hex_size * 0.6
        end_dist = hex_size * 0.85
        x1 = center_x + start_dist * math.cos(rad)
        y1 = center_y + start_dist * math.sin(rad)
        x2 = center_x + end_dist * math.cos(rad)
        y2 = center_y + end_dist * math.sin(rad)
        draw.line([(x1, y1), (x2, y2)], fill=grip_color, width=2)
...
def save_cursor_images(output_dir):
    ...
    create_drag_drop(32).save(f"{output_dir}/drag-drop.png")
    create_minimal_crosshair(32).save(f"{output_dir}/crosshair.png")
    create_minimal_move(32).save(f"{output_dir}/move.png")
...
cat > "drag-drop.cursor" << EOF
32 16 16 drag-drop.png
EOF
...
xcursorgen drag-drop.cursor "$CURSOR_DIR/dnd-move"
...
cd "$CURSOR_DIR"
ln -sf dnd-move dnd-copy
ln -sf dnd-move dnd-link
ln -sf dnd-move dnd-ask
ln -sf dnd-move dnd-none
